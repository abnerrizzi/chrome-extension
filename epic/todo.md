...
- [ ] ST-040 `extension/parsers/pncp_parser.js` DOM list parser
- [ ] ST-041 `extension/parsers/pncp_detail_parser.js` API detail parser
- [ ] ST-042 Registry in `background.js` (DOMAIN_REGISTRY), `manifest.json`, and `popup.js` (KNOWN_DOMAINS / previewFields)
- [ ] ST-043 JSON Schemas `pncp.json` (lista) and `pncp_detail.json` (detalhe)
- [ ] ST-044 Normalizer `normalization/pncp.py` + registration in `ingest.py`
- [ ] ST-045 Persistence mapping in `persistence.py` (both domains to `pncp_items`)
- [ ] ST-046 Liquibase migrations `modules/pncp.sql` (Postgres & SQLite trees) + includes in `master.xml`
- [ ] ST-047 Server-side fetcher `pncp_fetcher.py` + `pncp_parser.py` (app/scrape) and scrape router integration
- [ ] ST-048 Integration and unit tests for PNCP module
